...
void wyslij(string bigos)
{
	...
	if (!blocked) dowyslania.push_back(bigos);
}
...
void setCustomDamage(int id) {
	...
	if (!customDamage)
	{
		...
		customDamage.open(dataPath + "customDamage.txt", std::fstream::in | std::fstream::out);
		customDamage << "//1. knife 3.tekbow 4. pistol 5. magnum 7. chargeDart 8. shotgun 9.shueder 10. plasma 11. firestorm 13. cbore 15. grenadeLauncher 16. scorpionLauncher 17. harpoon 18. torpedo 20. flameThrower 22. nuke" << endl;
		customDamage << "//example: (custom damage for chargeDart)" << endl;
		customDamage << "//7; d: 2.0" << endl;
		customDamage << "//example: (custom node for nuke)" << endl;
		customDamage << "//22; n: 3.5" << endl;
		...
		customDamage << "//It should be mainly used to lower the dmg multiplier of some OP weapons. For example: global DamageMultiplier=1.20 is fine for almost every weapon but shotgun - so you can type below \"8; d: 1.0\"." << endl;
	...
	if (!customPickups)
	{
		...
		// re-open with original flags
		customPickups.open(dataPath + "customPickups.txt", std::fstream::in | std::fstream::out);
		customPickups << "//2.bow 3.tekbow 4.pistol 5. magnum 6.tranquilizer 7. chargeDart 8. shotgun 9.shueder 10. plasma 11. firestorm 12. sunfirePod 13. cbore 14. P.F.M. 15. grenadeLauncher 16. scorpionLauncher 17. harpoon 18. torpedo 20. flameThrower 21. razorWind 22. nuke" << endl;
		...
		customPickups << "//Chance: 22; 50" << endl;
		customPickups << "//Pick: 22; 10: 20" << endl;
		customPickups << "//Pick: 22; 21: 0" << endl;
		customPickups << "//Ammo: 22; 3" << endl;
					...
					if (tekst.find("return") == 7) {
						...
						if (tekst.find("join") == 7) {
							...
							if (!permissions)
							{
								...
								permissions << "//pdestroyer123; kick ;" << endl;
							}
							string dane;
							bool czy_continue = false;
							while (getline(permissions, dane)) {
								//cout << "dane: " << dane << endl;
								if (dane.find(login) == 0 && dane.find("kick ") != std::string::npos) {
									ss.str("");
							...
							while (getline(permissions, dane)) {
								//cout << "dane: " << dane << endl;
								if (dane.find(login) == 0 && dane.find("changelvl ") != std::string::npos) {
									int var = 1;
									...
									//ss.str("");
									//ss << " \x4/" << dec << zeranumer(id) << " news Changing level.";
									//wyslij(ss.str());
									czy_continue = true;
								}
							}
...
void serverRules() {
	...
	if (lvlSet.find("beta03.lsm") == std::string::npos) {
...
DWORD WINAPI Main_thread(LPVOID lpParam)
{
	...
	cout << "Bot (dll) injected successfully. b0.3" << endl;
	...
	do {
		for (auto it = waiting4game.begin(); it != waiting4game.end();) {
			...
			//if (fazagry == 0 || fazagry == 1) {
			//cout << "for waiting4game *it: " << *it << endl;
			ss.str("");
			...
			//waiting4game.erase(std::remove(waiting4game.begin(), waiting4game.end(), i), waiting4game.end()); //erase by value
